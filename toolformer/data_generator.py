...
# LOGFORMAT = logging.Formatter('%(asctime)s %(clientip)-15s %(user)-8s %(message)s')
# #logging.basicConfig(format=LOGFORMAT)
#
# logger = logging.getLogger(__name__)
# logger.addHandler(logging.StreamHandler(sys.stdout).setFormatter(LOGFORMAT))
...
class DataGenerator(nn.Module):
    ...
    def generate_api_candidates_and_baselines (
        ...
        for api_idx in api_positions:
            ...
            logging.info(f"prompt_and_generated_ids: {self.tokenizer.decode(prompt_and_generated_ids)}")
            ...
            with torch.no_grad():    
                while i < MAX_SEQ_LENGTH:
                    ...
                    if next_token == torch.tensor([self.api_end_token_id]).to(self.device):
                        ...
                        # inserts actual API response into text
                        modified_generation_ids = torch.cat([
                            modified_generation_ids,
                            torch.tensor([self.api_end_token_id]).to(self.device),
    ...
    def generate(
        ...
        for api in self.apis:
            ...
            logging.info("Samples API positions:")
